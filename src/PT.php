...
if ( ! class_exists( 'WPS\PostTypes\PT' ) ) {
	 ...
	 * @package WPS\PostTypes
	 */
	class PT extends WPS\Core\Registerable {
		 ...
		 */
		public function __construct( $post_type, array $args, $create = true ) {

			if ( did_action( 'init' ) ) {
				throw new \Exception( 'Initializing post type too late. Should be done in the init hook or before.' );
			}
		 ...
		 */
		protected function register_post_type( $args = array() ) {
			$args = ! empty( $args ) ? wp_parse_args( $args, $this->get_args() ) : $this->get_args();
